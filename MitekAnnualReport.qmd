...
The outline of this report has been derived from the [EuroSpine Registry](https://drive.google.com/file/d/1ahcwmg50p87BR0AgJQXb740z2g7LUj-J/view?usp=sharing "Link to report example") report supplied as an example by Bruce Robie (Mitek, JnJ MedTech) (Sep 2023) and via summary of meeting minutes provided by Tracey Mealey in email correspondence (26-Jun-2023). A variation of the report structure was added in April-2026 to align with Depuy-Mitek reporting requirements for continuous variables.
...
+--------------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| WORC Physical Question 3 | Question 3 of the Physical subscale of the WORC asks "How much weakness do you experience in your shoulder?" and is a key indicator or rotator cuff integrity, especially after repair.                                                                                              |
+--------------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
get_specific_snapshot <- function(folder_name, base_folder_id = base_folder_id1) {
  tryCatch({
    folder <- googledrive::drive_ls(
      path = googledrive::as_id(base_folder_id),
      q = paste0("name = '", folder_name, "' and trashed = false")
    )
    ...
    snapshot_file <- googledrive::drive_ls(
      path = googledrive::as_id(folder$id[1]),
      q = "name = 'Registry data snapshot.xlsx' and trashed = false"
    )
...
SheetIDs <- list(
  ...
  AuditReport = "https://docs.google.com/spreadsheets/d/1Z79F9LrotbYTf5F7pPlqpMwV8yk1K-Upy3ty7cu06UY/edit"
...
specific_snapshot <- get_specific_snapshot("20260616")
...
SnapshotDate <- lubridate::ymd("20260616")
...
SnapshotComb <- SnapshotRC |> dplyr::select(-(c(ExternalStudyTag,Email,Phone))) |>
  bind_rows(dplyr::select(SnapshotGHI, -(c(ExternalStudyTag,Email,Phone))) |>
      mutate(RepairAugmentation = as.character(RepairAugmentation))
...
#  ss = "https://docs.google.com/spreadsheets/d/1Z79F9LrotbYTf5F7pPlqpMwV8yk1K-Upy3ty7cu06UY/edit", 
...
ProcedureMaster1 <- ProcedureMaster |> left_join(
  ...
  dplyr::filter(!is.na(ReplaceProcedure), !(ReplaceProcedure == "none")) |> 
  ...
  # dplyr::filter out rows where ReplaceProcedure contains certain strings
  dplyr::filter(str_detect(ReplaceProcedure, "slap|bankart|augment*", negate = TRUE)) |> 
...
ProcedureMaster2 <- left_join(
  ...
  SnapshotComb1 |> dplyr::filter(SurgicalTreatment == "Surgical") |> dplyr::select(
...
ProductUsage1 <- SnapshotComb2a |> dplyr::select(TreatmentID,ImplantCodes) |> dplyr::filter(grepl("10886",ImplantCodes))
...
if (nrow(ProductUsage2 |> dplyr::filter(is.na(Description)) |> arrange(ImplantCodes)) < 1) {
  invisible(
    googlesheets4::range_write(
      ss = SheetIDs$AuditReport,
          data = ProductUsage2 |> dplyr::filter(is.na(Description)) |> arrange(ImplantCodes),
...
message_text <- if_else(
  nrow(ProductUsage2 |> dplyr::filter(is.na(Description)) |> arrange(ImplantCodes)) < 1,
  "There are no cases to flag for discrepancies.",
  "Discrepancies identified have been added to external table."
)

if (knitr::is_html_output()) {
  epoxy::epoxy_html(message_text)
} else {
  cat(message_text, "\n")
}
```
...
ProductUsage3 <- ProductUsage2 |> distinct(CustomUID, .keep_all = TRUE) |> dplyr::filter(grepl("10886",ImplantCodes)) |>
  rowwise() |>
...
SnapshotPROM1 <- SnapshotPROM |>
  pivot_longer(
    ...
    values_to = c("QDASH", "DeltaQ", "WOSINorm", "WOSIDelta", "Eligible")
  ) |>
  mutate(
    Timepoint = forcats::fct(
      Timepoint,
      levels = c("Preop", "3m", "6m", "12m")),
    Followup =  if_else(is.na(DateStatusChange),NA,as.numeric(as.duration(interval(DateTreatment, DateStatusChange)),"days"))
    ) |> dplyr::filter_out(
      is.na(Eligible),
      Timepoint == "12m" & Followup < 308
      ) |> rename(
      QDASHDelta = "DeltaQ"
    )
...
SnapshotPROM2 <- SnapshotPROM |>
  pivot_longer(
    ...
    values_to = c("WORCNorm","WORCDelta","WORCPhysicalQ3", "Eligible")
  ) |>
  mutate(Timepoint = forcats::fct(
    Timepoint, 
    levels = c("Preop", "6m", "12m"))
    ) |> dplyr::filter(!is.na(Eligible))
...
```{r}
#| label: tbl-ae-criteria
#| tbl-cap: "Adverse event classification criteria applied to free-text complication descriptions"
...
tibble::tibble(
  ...
  `Classifying Terms` = c(
    "washout, infect, antibiotic, VAC dressing, culture",
    "fail, recurrent, rupture, retear, torn, weak, weakness, tear",
    "capsulitis, frozen, stiff, release, arthrofibrosis",
    "regraft, loosening (excluding loose bodies), pullout",
    "removal, irritation, explant, broken, annoying, device, replacement",
    "instability, subluxation, dislocation",
    "palsy, radiculopathy, carpal tunnel",
    "fracture",
    "wound, necrosis",
    "DVT, embolism, PE, occlusion",
    "swelling, edema, oedema, effusion, swollen",
    "pain, complex regional pain syndrome (CRPS)",
    "Any complication not meeting criteria for any named category above"
  )
) |>
  knitr::kable(align = c("l", "l"))
```

A complication was flagged as **Other** when no named category criteria were met. A secondary flag, **Isolated Pain**, was assigned where **Pain** was the sole category identified --- distinguishing pain as a primary presenting complication from cases where pain co-occurred alongside another classified event.
...
There are {nrow(SnapshotComb2a |> dplyr::filter(is.na(TreatmentID)))} cases with missing TreatmentID in the dataset and {nrow(ComplicTable1 |> filter(is.na(TreatmentID)))} missing in the complications table.
...
ComplicTable2 <- ComplicTable1 |>
  dplyr::filter(!(TreatmentID %in% mismatched_rows$TreatmentID))
```
...
ComplicFailCheck <- ComplicTable3 |> dplyr::filter(
  !is.na(DateStatusChange)
) |> mutate(
...
ComplicInfection <- ComplicTable3 |> dplyr::group_by(TreatmentID) |> dplyr::filter(Infection == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicLigament_Tendon <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Ligament_Tendon == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicStiffness <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Stiffness == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicLoosening <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Loosening == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicHardware <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Hardware == "Yes" & Loosening == "No") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicInstability <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Instability == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicNeurological <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Neurological == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicFracture <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Fracture == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicThrombosis <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Thrombosis == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicEffusion <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Effusion == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicPain <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(PainIsol == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicOther <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(Other == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
ComplicReop <- ComplicTable3 |> group_by(TreatmentID) |> dplyr::filter(`Reoperation Procedure` == "Yes") |> slice_min(DateOccurFix, n = 1) |> ungroup()
...
ComplicDiscrep <- ComplicTable3 |> dplyr::filter(
  DateCheck == "Check"|
...
# Comment out after first use
 ...
 with_gs4_quiet(range_write(SnapshotComb2a |>
               dplyr::filter(SurgicalTreatment == "Surgical" & is.na(ComplicationOccurrence)) |>
...
TableRepB <-
  ...
  dplyr::filter(SurgicalTreatment == "Surgical" &
  ...
  dplyr::select(
    ...
    location = "column"
  ) |> gtsummary::add_overall() |> gtsummary::modify_table_styling(
      columns = label,
...
RefCode1 <- ProductTable |> dplyr::filter(
...
SelectRep1RC <- SelectRep1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
#SelectRep1RCno <- SelectRep1 |> dplyr::filter(str_detect(CuffRepair,"Repair",negate = TRUE) | is.na(CuffRepair))

SelectRep1RCPROM1 <- SelectRep1PROM1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
SelectRep1RCPROM2 <- SelectRep1PROM2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep1BT <- SelectRep1 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep1BTPROM1<- SelectRep1PROM1 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep1BTPROM2<- SelectRep1PROM2 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
```
...
#| tbl-cap: "Summary of PRULO Report 1 (Rotator Cuff) - Patient Characteristics"
...
TableRep1RC1 <- SelectRep1RC |> mutate(
  ...
  dplyr::select(
    ...
    tbl_summary(
      ...
      type = list(
        AgeAtInitialExam ~ "continuous2",
        SymptomDuration ~ "continuous2",
        Sex ~ "dichotomous",
        TreatmentStatus ~ "dichotomous",
...
CuffStatus1 <- SelectRep1RC |> dplyr::filter(is.na(CuffStatus))
```
...
TableRep1RC2 <-
  ...
  tbl_summary(
      ...
      columns = label,
      rows = label == "Other" & variable == "RepairAugmentation",
      footnote = "Biceps tendon integration; Biceps tendon transfer; Tendon advancement"
    ) |> modify_table_styling(
    columns = label,
    rows = variable == "CapsuleLigament" & row_type == "label",
    footnote = "Procedures affecting the joint capsule or ligaments"
  ) |> gtsummary::as_flex_table() |> 
  flextable::set_table_properties(
    width = 1, 
    layout = "autofit"
...
FailuresRep1RC <-
  SelectRep1RC |> mutate(
    TreatmentStatusDesc = stringr::str_to_sentence(TreatmentStatusNotes)
      ...
      dplyr::filter(TreatmentStatus == "Failed") |> 
  dplyr::select(
    TreatmentID,
    AgeAtInitialExam, 
    Sex,
...
message_text <- if_else(
  nrow(FailuresRep1RC) == 0,
  "There are no failures to report.",
  'Failures were written to <a href="{{SheetIDs$AuditReport}}">Attachment 1</a> for further review'
)

if (knitr::is_html_output()) {
  epoxy::epoxy_html(message_text)
} else {
  glue::glue(message_text)
}
...
SelectRep1Other <- SelectRep1 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
has_complications <- SelectRep1RC |> 
  dplyr::filter(TreatmentID %in% IntraopComp$TreatmentID) |> 
  nrow() > 0

# Conditionally generate content
if (!has_complications) {
  # If no complications, return the text message
  glue::glue("No intraoperative complications observed.")
} else {
  ...
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep1RC$TreatmentID) |> 
    rename(
      Description = "ICNature4",
...
Postoperative complications were recoded from report text where definitions were met and the results summarised.
...
SelectRep1RCReop <- ComplicTable3 |> dplyr::filter(
...
```{r}
#| label:  Reoperations-Report-Rep1A-write
#| eval: false
#| include: false
...
message_text <- if_else(
  nrow(SelectRep1RCReop) == 0,
  "There are no reoperations to report.",
  'Reoperations were written to <a href="{{SheetIDs$AuditReport}}">Attachment 1</a> for further review'
)
...
TableRep1RC6 <- SelectRep1RCPROM1 |>
dplyr::select(
  QDASH,
  QDASHDelta,
  Timepoint) |>
...
```{r}
#| label: tbl-PROMs-Rep1_b
#| tbl-cap: "Summary of PRULO Report 1 (Rotator Cuff) Cases - QuickDASH"


# Table 1: QDASH across all timepoints including Preop
tbl_qdash <- SelectRep1RCPROM1 |>
  dplyr::select(QDASH, Timepoint) |>
  tbl_summary(
    by = "Timepoint",
    missing = "no",
    type = list(QDASH ~ "continuous2"),
    statistic = list(
      all_continuous2() ~ c(
        "{mean} ({sd})",
        "{median} ({p25}, {p75})",
...
# Table 2: QDASHDelta excluding Preop timepoint
tbl_qdash_delta <- SelectRep1RCPROM1 |>
  dplyr::filter(Timepoint != "Preop") |>
  dplyr::select(QDASHDelta, Timepoint) |>
  tbl_summary(
    by = "Timepoint",
    missing = "no",
    type = list(QDASHDelta ~ "continuous2"),
    statistic = list(
      all_continuous2() ~ c(
        "{mean} ({sd})",
        "{median} ({p25}, {p75})",
...
TableRep1RC6b <- tbl_stack(
  quiet = TRUE,
  list(tbl_qdash, tbl_qdash_delta)
) |>
  gtsummary::as_flex_table() |>
  flextable::set_table_properties(
    width = 1,
...
knitr::knit_print(TableRep1RC6b)
...
TableRep1RC7 <- SelectRep1RCPROM2 |>
...
tbl_summary(by = "Timepoint",
            missing = "no",
            type = list(
              WORCNorm ~ "continuous2",
              WORCPhysicalQ3 ~ "continuous2",
              WORCDelta ~ "continuous2"
            ),
            statistic = all_continuous2() ~ c(
        "{mean} ({sd})",
        "{median} ({p25}, {p75})",
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep1BT$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep1BTReop <- ComplicTable3 |> dplyr::filter(
...
# There are some rogue records with PROMs captured for the same record after the datestatus change (failed etc). These are removed manually

SelectRep2PROM1 <- SnapshotPROM1 |>
  dplyr::filter(
    str_detect(ImplantCodes,paste(RefCode2$Reference,collapse = "|")),
    !(TreatmentID == "17.1"),
    !(TreatmentID == "281.1" & Timepoint == "12m"),
    !(TreatmentID == "816.2" & Timepoint == "12m")
  )
...
SelectRep2RC <- SelectRep2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep2RCPROM1 <- SelectRep2PROM1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
SelectRep2RCPROM2 <- SelectRep2PROM2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep2BT <- SelectRep2 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep2BTPROM1<- SelectRep2PROM1 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep2BTPROM2<- SelectRep2PROM2 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
```
...
CuffStatus2 <- SelectRep2RC |> dplyr::filter(is.na(CuffStatus))
```
...
SelectRep2Other <- SelectRep2 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
if (!has_complications) {
  # If no complications, return the text message
  glue::glue("No intraoperative complications were observed.")
} else {
  # Calculate incidence and confidence intervals
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep2RC$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep2RCReop <- ComplicTable3 |> dplyr::filter(
...
colnames(### Patient-Reported Outcomes
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep2BT$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep2BTReop <- ComplicTable3 |> dplyr::filter(
...
RefCode3 <- ProductTable |> dplyr::filter(
  (Category == "Anchor"|Category == "Anchor + Suture") &
  stringr::str_detect(stringr::str_to_lower(Description),"advance") &
...
```{r}
#| label:  procedure-selection-Rep3

SelectRep3RC <- SelectRep3 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep3RCPROM1 <- SelectRep3PROM1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
SelectRep3RCPROM2 <- SelectRep3PROM2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep3BT <- SelectRep3 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep3BTPROM1<- SelectRep3PROM1 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep3BTPROM2<- SelectRep3PROM2 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
```
...
CuffStatus2 <- SelectRep3RC |> dplyr::filter(is.na(CuffStatus))
```
...
if (!has_complications) {
  cat("No intraoperative complications observed.")
} else {
  ...
  prop_result <- binom::binom.confint(complication_cases, total_cases, methods = "wilson")
  incidence_pct <- round(prop_result$mean * 100, 1)
  ci_lower      <- round(prop_result$lower * 100, 1)
  ci_upper      <- round(prop_result$upper * 100, 1)
  
  cat(glue::glue(
    "Intraoperative events occurred in {complication_cases} of {total_cases} cases ",
    "({incidence_pct}%, 95% CI: {ci_lower}%\u2013{ci_upper}%).\n\n"
  ))
  
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep3RC$TreatmentID) |> 
    dplyr::rename(
      Description      = ICNature,
      Intervention     = ICIntervention,
      PostopManagement = ICPostopManagement
    ) |> 
    dplyr::select(TreatmentID, Description, Intervention, PostopManagement) |> 
    gt::gt()
}
...
SelectRep3Other <- SelectRep3 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep3RCReop <- ComplicTable3 |> dplyr::filter(
...
TableRep3BT2 <-
  ...
  tbl_summary(
    ...
    missing = "no") |> modify_table_styling(
      columns = label,
      rows = label == "Other" & variable == "LabrumRepair",
      footnote = "Biceps tendon integration; Biceps tendon transfer; Tendon advancement"
    ) |> modify_table_styling(
    columns = label,
    rows = variable == "CapsuleLigament" & row_type == "label",
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep3BT$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep3BTReop <- ComplicTable3 |> dplyr::filter(
...
RefCode4 <- ProductTable |> dplyr::filter(
  (Category == "Anchor + Suture") &
  stringr::str_detect(stringr::str_to_lower(Description),"advance") &
  stringr::str_detect(stringr::str_to_lower(Description),"br") &
  stringr::str_detect(stringr::str_to_lower(Description),"healix") &
...
SelectRep4RC <- SelectRep4 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep4RCPROM1 <- SelectRep4PROM1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
SelectRep4RCPROM2 <- SelectRep4PROM2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep4BT <- SelectRep4 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep4BTPROM1<- SelectRep4PROM1 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep4BTPROM2<- SelectRep4PROM2 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
```
...
CuffStatus2 <- SelectRep4RC |> dplyr::filter(is.na(CuffStatus))
```
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep4RC$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep4Other <- SelectRep4 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep4RCReop <- ComplicTable3 |> dplyr::filter(
...
```{r}
#| label:  Failure-Report-Rep4B-display

message_text <- if_else(
  nrow(FailuresRep4BT) == 0,
  "There are no failures to report.",
  'Failures were written to <a href="{{SheetIDs$AuditReport}}">Attachment 1</a> for further review'
)
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep4BT$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep4BTReop <- ComplicTable3 |> dplyr::filter(
...
RefCode5 <- ProductTable |> dplyr::filter(
  (Category == "Anchor"|Category == "Anchor + Suture") &
  stringr::str_detect(stringr::str_to_lower(Description),"advance") &
  stringr::str_detect(stringr::str_to_lower(Description),"br") &
...
SelectRep5RC <- SelectRep5 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep5RCPROM1 <- SelectRep5PROM1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
SelectRep5RCPROM2 <- SelectRep5PROM2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep5BT <- SelectRep5 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep5BTPROM1<- SelectRep5PROM1 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep5BTPROM2<- SelectRep5PROM2 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
```
...
CuffStatus2 <- SelectRep5RC |> dplyr::filter(is.na(CuffStatus))
```
...
TableRep5RC2 <-
  ...
  tbl_summary(
      ...
      columns = label,
      rows = label == "Other" & variable == "CapsuleLigament",
      footnote = "Procedure not involving a repair (e.g. capsulotomy)"
    ) |> modify_table_styling(
    columns = label,
    rows = variable == "CapsuleLigament" & row_type == "label",
    footnote = "Procedures affecting the joint capsule or ligaments"
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep5RC$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep5Other <- SelectRep5 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep5RCReop <- ComplicTable3 |> dplyr::filter(
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep5BT$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep5BTReop <- ComplicTable3 |> dplyr::filter(
...
RefCode6 <- ProductTable |> dplyr::filter(
  (Category == "Anchor"|Category == "Anchor + Suture") &
...
SelectRep6BR <- SelectRep6 |> dplyr::filter(LabrumRepair == "Bankart")
SelectRep6BRPROM1 <- SelectRep6PROM1 |> dplyr::filter(LabrumRepair == "Bankart")

SelectRep6SLAP <- SelectRep6 |> dplyr::filter(LabrumRepair == "SLAP")
SelectRep6SLAPPROM1<- SelectRep6PROM1 |> dplyr::filter(LabrumRepair == "SLAP")
...
SelectRep6Other <- SelectRep6 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep6Reop <- ComplicTable3 |> dplyr::filter(
  TreatmentID %in% SelectRep6$TreatmentID,
  `Reoperation Procedure` == "Yes"
  ) |> group_by(TreatmentID) |> slice_min(
    ReopDelay_Surgery,
...
tbl_summary(by = "Timepoint",
            ...
            type = list(
              QDASH ~ "continuous2",
              QDASHDelta ~ "continuous2",
              WOSINorm ~ "continuous2",
              WOSIDelta ~ "continuous2"
            ),
            statistic = all_continuous2() ~ c(
        "{mean} ({sd})",
        "{median} ({p25}, {p75})",
...
TableRep6BR2 <-
  ...
  tbl_summary(
      ...
      columns = label,
      rows = label == "Other",
      footnote = "Procedure other than repair (e.g. capsulotomy)"
    ) |>
  modify_table_styling(
    columns = label,
    rows = variable == "CapsuleLigament" & row_type == "label",
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep6BR$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep6BROther <- SelectRep6BR |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep6BRReop <- ComplicTable3 |> dplyr::filter(
...
RefCode7 <- ProductTable |> dplyr::filter(
  (Category == "Anchor + Suture") &
...
SelectRep7 <- SnapshotComb4 |> 
  dplyr::filter(
    ...
    RepairAugmentation = dplyr::case_when(
    is.na(RepairAugmentation) ~ "None",
    .default = as.character(RepairAugmentation)
    ),
    Product1Count = case_when(
      Product1Check == TRUE ~ stringr::str_extract(Product1,"\\d"),
      .default = NA
...
SelectRep7BR <- SelectRep7 |> dplyr::filter(LabrumRepair == "Bankart")
SelectRep7BRPROM1 <- SelectRep7PROM1 |> dplyr::filter(LabrumRepair == "Bankart")

SelectRep7SLAP <- SelectRep7 |> dplyr::filter(LabrumRepair == "SLAP")
SelectRep7SLAPPROM1<- SelectRep7PROM1 |> dplyr::filter(LabrumRepair == "SLAP")
...
SelectRep7Other <- SelectRep7 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep7Reop <- ComplicTable3 |> dplyr::filter(
  TreatmentID %in% SelectRep7$TreatmentID,
  `Reoperation Procedure` == "Yes"
  ) |> group_by(TreatmentID) |> slice_min(
    ReopDelay_Surgery,
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep7BR$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep7BROther <- SelectRep7BR |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep7BRReop <- ComplicTable3 |> dplyr::filter(
...
RefCode8 <- ProductTable |> dplyr::filter(
  (Category == "Anchor + Suture") &
  stringr::str_detect(stringr::str_to_lower(Description),"dynacord") &
  stringr::str_detect(stringr::str_to_lower(Description),"br") &
  stringr::str_detect(stringr::str_to_lower(Description),"gryphon")  
...
SelectRep8BR <- SelectRep8 |> dplyr::filter(LabrumRepair == "Bankart")
SelectRep8BRPROM1 <- SelectRep8PROM1 |> dplyr::filter(LabrumRepair == "Bankart")

SelectRep8SLAP <- SelectRep8 |> dplyr::filter(LabrumRepair == "SLAP")
SelectRep8SLAPPROM1<- SelectRep8PROM1 |> dplyr::filter(LabrumRepair == "SLAP")
...
RefCode9 <- ProductTable |> dplyr::filter(
  (Category == "Anchor + Suture") &
  stringr::str_detect(stringr::str_to_lower(Description),"dynacord") &
  stringr::str_detect(stringr::str_to_lower(Description),"peek") &
  stringr::str_detect(stringr::str_to_lower(Description),"gryphon")  
...
SelectRep9BR <- SelectRep9 |> dplyr::filter(LabrumRepair == "Bankart")
SelectRep9BRPROM1 <- SelectRep9PROM1 |> dplyr::filter(LabrumRepair == "Bankart")
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep9$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
#| tbl-cap: "Summary of PRULO Report 9 (All) Cases - Postoperative Events"
...
SelectRep9Reop <- ComplicTable3 |> dplyr::filter(
...
RefCode10 <- ProductTable |> dplyr::filter(
...
#| tbl-cap: "Summary of PRULO Report 10 (All) - Procedure Survival"
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep10$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep10Other <- SelectRep10 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep10Reop <- ComplicTable3 |> dplyr::filter(
...
RefCode11 <- ProductTable |> dplyr::filter(
  (Category == "Suture") &
 stringr::str_detect(stringr::str_to_lower(Description),"dynacord") 
)
...
SelectRep11RC <- SelectRep11 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep11RCPROM1 <- SelectRep11PROM1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
SelectRep11RCPROM2 <- SelectRep11PROM2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep11BT <- SelectRep11 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep11BTPROM1<- SelectRep11PROM1 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
SelectRep11BTPROM2<- SelectRep11PROM2 |> dplyr::filter(str_detect(LongHeadBiceps,"Tenodesis"))
```
...
CuffStatus2 <- SelectRep11RC |> dplyr::filter(is.na(CuffStatus))
```
...
# Do the write operation silently
if (nrow(FailuresRep11RC) > 0) {
  invisible(
    googlesheets4::range_write(
 ss = SheetIDs$AuditReport,
 data = FailuresRep11RC |> arrange(TreatmentID),
 sheet = "Failures11",
 range = "A1:P",
 col_names = TRUE
    )
...
if (!has_complications) {
  ...
  # Generate and return the table
  IntraopComp |> 
    dplyr::filter(TreatmentID %in% SelectRep11RC$TreatmentID) |> 
    rename(
      Description = "ICNature4",
      Intervention = "ICIntervention",
      PostopManagement = "ICPostopManagement"
...
SelectRep11Other <- SelectRep11 |> dplyr::filter(
  Other == TRUE
) |> dplyr::select(
  TreatmentID,
  DateTreatment,
...
SelectRep11RCReop <- ComplicTable3 |> dplyr::filter(
...
dplyr::select(WORCNorm,
              WORCPhysicalQ3,
              # WORCDelta, insufficient sample size
              Timepoint) |>
tbl_summary(by = "Timepoint",
            missing = "no",
            type = list(
...
RefCode12 <- ProductTable |> dplyr::filter(
  (Category == "Suture") &
...
SelectRep12RC <- SelectRep12 |> dplyr::filter(str_detect(CuffRepair,"Repair"))

SelectRep12RCPROM1 <- SelectRep12PROM1 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
SelectRep12RCPROM2 <- SelectRep12PROM2 |> dplyr::filter(str_detect(CuffRepair,"Repair"))
```
...
RefCode13 <- ProductTable |> dplyr::filter(
  (Category == "Suture") &
 stringr::str_detect(stringr::str_to_lower(Description),"orthocord") 
)
...
```{r}
#| label: productusage
#| code-summary: "Summary of product overlap for each report sample (row), the proportion of cases within that sample that also include products included in the remaining reports (columns)"
...
for (i in 1:ncol(SelectRepAll1)) {
  ...
  filtered_data <- SelectRepAll1 |> 
    dplyr::filter(SelectRepAll1[[i]] == 1)
...
A cross-tabulation of usage for all products was generated and written to [Attachment 1](https://docs.google.com/spreadsheets/d/1Z79F9LrotbYTf5F7pPlqpMwV8yk1K-Upy3ty7cu06UY/edit?usp=sharing) for further review.
